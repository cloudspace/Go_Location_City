...
func main() {
	...
	connectionURI := "host=127.0.0.1 port=5432 user=docker password=docker dbname=quattroshapes"
	query := fmt.Sprintf("SELECT qs_loc FROM qs_localities ORDER BY ST_Distance(geom, ST_GeometryFromText('POINT(%f %f)', 4326)) limit 1", lng, lat)
...
func getJSONResultOfQuery(sqlString string, db *sql.DB) (string, error) {
	...
	if len(tableData) == 0 {
		...
		result["city"] = tableData[0]["qs_loc"]
		result["error"] = ""
	}
